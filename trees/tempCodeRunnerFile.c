...
struct node *rightRotation(struct node *p){
    ...
    a->rcptr=p;
    p->lcptr=ar;
    a->height=max(getHeight(a->lcptr), getHeight(a->rcptr))+1;
    p->height=max(getHeight(p->lcptr), getHeight(p->rcptr))+1;
...
struct node *leftRotation(struct node *p){
    ...
    p->rcptr=bl;
    b->lcptr=p;
     b->height=max(((b->lcptr != NULL) ? getHeight(b->lcptr) : 0), ((b->rcptr != NULL) ? getHeight(b->rcptr) : 0))+1;
    p->height=max(((p->lcptr != NULL) ? getHeight(p->lcptr) : 0), ((p->rcptr != NULL) ? getHeight(p->rcptr) : 0))+1;
    return b;
}
struct node *insert(struct node *n, int info){
    ...
    n->height = 1 + max(getHeight(n->lcptr), getHeight(n->rcptr));
